...
//
// It lives in `lib/` because both sides need the same answer as a VALUE: the
// panel draws the groups, and the chat line that introduces a board tallies them
// the same way (quoting the ranker's own count said "1 I'd pass on" beside a
// pile of forty, since the ranker never saw the roles the earlier passes had
// already ruled out). The board's payload loader is server code that opens a
// database connection, so a client component reaching in for this map pulled the
// Prisma runtime into the browser bundle and the whole dashboard route stopped
// compiling — a vocabulary two tiers share belongs in neither of them.
//
// Nothing but `pnpm run build` catches that: tsc resolves the module happily and
// ESLint's boundaries plugin governs `src/server/` internals, not the client
// edge. Same reason panelMode.ts sits apart from chatStore.
