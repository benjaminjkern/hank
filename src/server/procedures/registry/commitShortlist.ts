// Commit the shortlist board, then record where the user overruled Hank.
//
...
//
// The signal is relayed into the transcript as a Hank-only note and read by
// the ordinary consolidation pass at the NEXT segment wrap, rather than
// consolidated here or by a bespoke sub-agent. Three reasons: memory writes
// have exactly one home; the consolidator can see the whole conversation
// around the override — the user's own words when they gave a reason ("I'm not
// interested in security"), and nothing but the move itself when they didn't;
// and consolidation is a ~30s LLM pass, which inline here held every
// overridden commit's board on screen for that long (and double-consolidated
// whenever the company ended moments later). The note is a persisted row, so
// nothing is lost by waiting for the wrap.
...
export async function runCommitShortlist(
  ...
  if (overrides.length > 0 || revivals.length > 0) {
    ...
    // here so the wrap's consolidation pass has the corrections stated plainly
