...
| `add_more_companies` | `{answer: "yes" \| "no", settled: number}` — emitted once an add finishes. "Find more" (`yes`) re-enters discovery; "Done adding" (`no`) wraps the segment when `settled > 0` and falls through to what's next. Dispatched top-level. |
