...
| --- | --- |
| `company_checklist` | `{picked: Array<{name, context?, url?}>}` — emitted by the discovery arm; `context`/`url` ride back so the URL hunter can disambiguate. Dispatched top-level → `runChecklistAdd`. |
...
| `shortlist/` | seed-or-reshow (`runShortlist`): `loadShortlistJobsInput` → `shortlistJobs` → `seedBoardStances` → board on screen; the negotiation is chat + panel edits, ended by `commit_shortlist` ([shortlist-board.md](flows.md)) | — | company arm (`direction` forces a fresh seed) |
