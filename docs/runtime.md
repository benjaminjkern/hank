...
5. **Widget-submission shortcuts (deterministic, before any LLM call)** — [`dispatchTopLevelSubmission`](../src/server/widgets/dispatchTopLevelSubmission.ts). Each is parsed off the message marker by [`widgets/parse.ts`](../src/server/widgets/parse.ts) and dispatched at the top level, because the user's click already encodes the destination. In order: `company_disambiguation` → `runDisambiguationResolution` — `terminal`, because it ends by asking `add_more_companies`; `add_more_companies` → `ask` for "Find more" (Hank asks what to look for this time rather than re-searching blind), or for "Done adding" the segment wrap (when the round settled anything) then `consumed`, which is what falls through to what's next; `next_company_picker` → [`dispatchNextCompanyPicker`](../src/server/widgets/dispatchNextCompanyPicker.ts), which for an entity narrates "Picking up X." and hands its `entryTarget` to the turn below, and for `add_companies` returns `ask`. It reports back one of five outcomes — `none` (not a submission), `terminal` (the turn ends here), `enter` (run the turn on this `entryTarget`), `consumed` (the message is spent; fall through to what's-next), `ask` (Hank opens the next turn on the supplied `openingNudge`) — and that outcome is the only thing the chat runner branches on.

**Why `ask` is its own outcome and not an `enter` with no target.** An absent `entryTarget` means "nothing to dispatch on", which the state machine answers by yielding "Pulling up what's next…" and re-rendering the picker — correct for a stale row that vanished between render and submit, and exactly wrong for a click whose meaning was "ask me about this". Routed as `enter`, picking **Add companies to watchlist** re-drew the picker 142ms after its own opener and looked like a dead button. `ask` threads an `openingNudge` (a synthetic, never-persisted first user turn, parenthesized so Hank reads it as an instruction) through `runChat` into `runChatTurn`, which takes the Hank path instead of the silent-entry one. Profile intake uses the same field for the same reason. (Shortlist decisions don't come through widget submissions anymore: the user's board edits are panel POSTs relayed via `panel_edits` blocks, and the commit is Hank's `commit_shortlist` — see [flows.md](flows.md).)
...
| [`find_companies`](../src/server/agent/tools/registry/findCompanies.ts) | The discovery arm (`entryTarget: {kind:"discovery", direction, append}`), which opens the discovery panel. `append` keeps the batch on screen and writes alongside it (same `runId`); the default replaces it. |
| [`commit_discovery`](../src/server/agent/tools/registry/commitDiscovery.ts) | The discovery-commit arm (`entryTarget: {kind:"discovery_commit"}`) — adds every still-checked company, records the unchecked ones, then drops the panel back to the dashboard. The second handoff that also writes. |
...
| [`commit_shortlist`](../src/server/agent/tools/registry/commitShortlist.ts) | The walkthrough's continuation — applies the board's stances (picks → shortlist, borderline → set aside, passes → closed) and lands on the role picker, or the caught-up settle when nothing was picked. The one handoff that also **writes**, because committing the board IS entering the continuation. Its entry target carries `continuation: true`, so the company arm never re-scrapes on the way through — settling a round must not start a new one. |
...
**Company-event side effects.** These also emit a [`CompanyEvent`](../src/server/entities/companies/logCompanyEvent.ts) (the per-(user, company) timeline behind "Recent activity"): `closeCompany` → a `JOBS_CLOSED` summary + a `CLOSED` status row; the others → their status row (CAUGHT_UP only when the derived status actually lands there). Best-effort — never blocks the mutation. Batch seams elsewhere collapse the same way (one `SCRAPE_FOUND` per scrape, one `JOBS_CLOSED` per delist batch, one `SHORTLIST_RAN` per commit — the scan writes NO close event, since its skips are proposed stances, not closes), while per-role milestones dual-write a company row carrying `jobId`+`jobTitle`. Full vocabulary: [lifecycle.md → Company events](lifecycle.md).
...
| --- | --- |
| `company_checklist` | REPLAY-ONLY ([INCOMPLETE_MIGRATIONS.md](INCOMPLETE_MIGRATIONS.md)) — the discovery panel replaced it. Picking companies is panel marks relayed as `panel_edits`, settled by Hank's `commit_discovery`. |
...
| `add_more_companies` | `{answer: "yes" \| "no", settled: number}` — emitted once an add finishes. "Find more" (`yes`) hands Hank the turn to ask what this round should look for; "Done adding" (`no`) wraps the segment when `settled > 0` and falls through to what's next. Dispatched top-level. |
...
| `shortlist/` | rank-the-pool (`runShortlist`): `loadShortlistJobsInput` → `shortlistJobs` → `seedBoardStances` → board on screen — a fresh seed, or a merge of late-arriving pool rows into an open board; re-show only when the pool is empty. The negotiation is chat + panel edits, ended by `commit_shortlist` ([flows.md](flows.md)) | — | company arm (`direction` steers a fresh ranking) |
...
| `consolidateSessionMemory.ts` + `compactSession.ts` | `memoryConsolidation` → memory writes; then `compactSummary` → persist summary + advance the cutoff | — | `runWrapSegment`, `runCommitProfile` |
| `wrapSegment.ts` | the two above, in order | — | `runChat`, on `endedCompanyId` |
| `commitProfile.ts` | consolidate → verdict gate → compact | `commit_profile` | — |

**Naming.** A procedure's folder/file and its entry export are one name — `preScan/` → `runPreScan`, `wrapSegment.ts` → `runWrapSegment` — so a folder has exactly one `run<FolderName>`. A second entry into the same procedure is named for what it does (`runChecklistAdd`), and a generator's drain-to-completion wrapper is `<entry>Collect`.
