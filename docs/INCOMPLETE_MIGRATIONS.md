...
- **Replay-only `company_checklist`** — the discovery panel (docs/flows.md) replaced the in-chat checklist; nothing emits, submits, or dispatches the kind anymore. What survives, solely because persisted `pipeline_widget` blocks + submission markers in old sessions carry it: the entry in the `WidgetKind` union, a `toText`-only def in the widget registry (it grounds model replay/scrollback), and a null-rendering component (a pre-panel session whose LAST message was a live checklist mounts one and gets nothing — asking for companies again opens the panel). A stale queued marker sent post-deploy falls through the dispatcher to Hank as plain text. Same shape and same decision as the shortlist family above: rewrite the history blocks to a tombstone shape and delete all of it, or accept permanently.
