...
| URL | Panel mode |
| --- | --- |
| `/` (`/dashboard` is an alias, normalized to `/`) | `dashboard` |
...
1. **Reach for the existing primitive before writing markup**: `statusColor`/`statusTone` ([statusColors.ts](../src/lib/statusColors.ts)), `companyLogoUrl` + the logo-chip template (`<img>` + `onError`→`initial`), `relativeTime` ([date.ts](../src/utils/date.ts)), and the [shared/](../src/components/RightPanel/shared/) pieces — [`RecentActivity`/`RecentActivityList`](../src/components/RightPanel/shared/RecentActivity.tsx) (timelines; headless-inner + chrome-wrapper split so a surface can BYO chrome) [`useExpandable(items, previewCount)`](../src/components/RightPanel/shared/useExpandable.ts) (preview/expand bookkeeping — returns `{visible, expanded, truncated, canCollapse, hiddenCount, expand, collapse, toggle}`; pass `items.length` to opt out of capping), and [`ContactCard`](../src/components/RightPanel/shared/ContactCard.tsx) (one person — name, `role @ agency via channel` subline, mailto/tel/LinkedIn links, notes; owns only the row and its dashed separator, so a list is N of them and the parent supplies the Card chrome. Takes the shared [`ContactView`](../src/server/views/contactView.ts), whose `CONTACT_SELECT` is `satisfies`-tied to the type so a new field can't be declared without being fetched).
