...
| `resume.md`               | User states a background FACT no résumé carried, or corrects one — including one they settle by rewriting an answer the [read-back](flows.md) flagged | "the platform team was 4 people, not 40"; rewriting a letter that called a still-running venture past-tense |
| `frequent_questions.md`   | User types out an answer to an application-style question, **or submits an application** (every answer sent counts as used, whoever drafted it — they put their name on it). The test is whether another employer would ask the same thing in roughly the same words, not whether it's been seen twice | "for 'why this company' I usually say…"; a submitted "what's your notice period" |
