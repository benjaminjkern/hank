...
| `SCANNING`  | notStarted | Entered: reading the board and triaging roles. Nothing owed by the user yet.                    | Hank (the passes)                 |
...
**Intended flow:** `NEW → READY → SCANNING → SHORTLISTING → APPLYING → {IN_FLIGHT | IN_PROCESS | CAUGHT_UP}`, with `PAUSED` / `BLOCKED` / `CLOSED` as set-asides off any state.
...
- `SCANNING` / `SHORTLISTING` / `APPLYING` split the stretch between entering a company and finishing with it, because they answer different questions about whose move it is. **SCANNING** is written on entry (the next-company picker, `resumeCompany`) via `markCompanyScanning` — the board is being read and nothing is asked of the user, so it takes the `notStarted` tone and the "Not started" bucket. **SHORTLISTING** is written by the shortlist seed (`markCompanyShortlisting`) the moment the board opens: this is the one stretch where the next move is genuinely the user's, which is what earns it a status rather than a phase of SCANNING. **APPLYING** now means the shortlist is committed and the survivors are being worked — set by `commitShortlist`, manually held, job events don't auto-move it. All three are "ongoing work" to `whats_next` rung 1.
