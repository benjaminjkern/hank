...
| `SHORTLISTED`         | focusNow   | User approved the pick; ready to apply.                                        | User (write + submit)             |
...
| `READY`     | notStarted | Scanned, PRE_SCAN passed with survivors; walkthrough not started.                             | User / Hank starts the walkthrough |
| `APPLYING`  | focusNow   | Walkthrough in progress — reading descriptions, shortlisting, drafting.                        | User (work the round)             |
...
| --------------- | ----------- | ------------- | -------------------------------------------------------------------------------------------- |
| **Now**         | focusNow    | yes           | `APPLYING` companies, companies with a focusNow job (`INTERVIEW_DEBRIEF` / `OFFERED`), focusNow Opportunities. |
...
**Intended flow:** `NEW → SCANNED → (SHORTLISTED | CLOSED | DEFERRED) → APPLIED → RESPONDED → INTERVIEW_SCHEDULED → INTERVIEW_DEBRIEF → WAITING_ON_RESPONSE → OFFERED` for scan-flow jobs; `PITCHED → (SCANNED | CLOSED | DEFERRED) → SHORTLISTED → APPLIED → …` for opportunity-linked roles.
...
- `DELISTED` is **system-set** by the closure detection in [upsertScrapedJobs.ts](../src/server/entities/jobs/upsertScrapedJobs.ts) (a re-scrape no longer returns the posting): stamps global `Job.closedAt`, flips only the non-terminal set (NEW/SCANNED/SHORTLISTED/DEFERRED) and logs a `DELISTED` job event per flipped row (`occurredAt = closedAt`), leaves applied/interviewing history intact, and guards against a flaky partial scrape (skips on empty scrape or >50% drop on a non-trivial board). Terminal; excluded from the gates and `whats_next` (`list_jobs` lists every status now — no default filter).
...
**Intended flow:** `NEW → READY → APPLYING → {IN_FLIGHT | IN_PROCESS | CAUGHT_UP}`, with `PAUSED` / `BLOCKED` / `CLOSED` as set-asides off any state.
...
- `APPLYING` — set manually when starting the round (canonical), auto-bumped from READY/NEW/CAUGHT_UP/IN_FLIGHT/IN_PROCESS on a SHORTLISTED commit (safety net). Manually held; job events don't auto-move it.
